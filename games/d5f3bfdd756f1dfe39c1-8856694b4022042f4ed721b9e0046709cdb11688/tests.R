...
s = rep(1, N)
  ...
  get.coords <- reactive({
    if(is.null(input$coords)) data.frame(x=x[input$i], y=y[input$i])
    else data.frame(x=input$coords$x, y=input$coords$y)
  })

    i = 10
    temp <- data.frame(x=x[input$i], y=y[input$i]
    xval <- temp$x
    yval <- temp$y
  ...
  output$hoverinfo <- renderTable({
    if (is.null(input$hover))
      ...
      data.frame(x=input$hover$x, y=input$hover$y)
  })
})
